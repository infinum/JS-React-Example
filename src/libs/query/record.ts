import { Dict, isNone, clone } from './utils';
...
export function equalRecordIdentities(record1?: RecordIdentity | null, record2?: RecordIdentity | null): boolean {
	return (
		(isNone(record1) && isNone(record2)) ||
		(!!record1 && !!record2 && record1.type === record2.type && record1.id === record2.id)
	);
}
...
export function uniqueRecordIdentities(set1: RecordIdentity[], set2: RecordIdentity[]): RecordIdentity[] {
	return exclusiveIdentities(serializeRecordIdentities(set1), serializeRecordIdentities(set2)).map((id) =>
		deserializeRecordIdentity(id)
	);
}

export function dedupeRecordIdentities(recordIdentities: RecordIdentity[]): RecordIdentity[] {
	let dict: Dict<true> = {};
...
export function mergeRecords(current: InitializedRecord | null, updates: InitializedRecord): InitializedRecord {
	if (current) {
...
function mergeRecordSection(record: any, current: any, update: any, section: string, replacementDepth: number): void {
...
export function isRecordIdentity(identity: RecordIdentity | unknown): identity is RecordIdentity {
