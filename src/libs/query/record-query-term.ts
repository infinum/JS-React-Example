...
export class FindRelatedRecordsTerm extends QueryTerm<FindRelatedRecords> {
	...
	page(param: PageQBParam): this {
		this._expression.page = pageParamToSpecifier(param);
	...
	filter(...params: FilterQBParam[]): this {
		...
		this._expression.filter = (this._expression.filter || []).concat(specifiers);
...
export type RecordQueryTerm = FindRecordTerm | FindRelatedRecordTerm | FindRelatedRecordsTerm | FindRecordsTerm;
