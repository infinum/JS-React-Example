import { QueryBuilderFunc } from './query';
...
import { FindRecordsTerm, FindRecordTerm, FindRelatedRecordsTerm, FindRelatedRecordTerm } from './record-query-term';

export type RecordQueryBuilderFunc = QueryBuilderFunc<RecordQueryExpression, RecordQueryBuilder>;
...
export class RecordQueryBuilder<RT = string, RI = RecordIdentity> {
	...
	findRecords(typeOrIdentities?: RT | RI[]): FindRecordsTerm {
		if (Array.isArray(typeOrIdentities)) {
			return new FindRecordsTerm(typeOrIdentities.map((ri) => this.normalizeRecordIdentity(ri)));
	...
	findRelatedRecord(record: RI, relationship: string): FindRelatedRecordTerm {
		return new FindRelatedRecordTerm(this.normalizeRecordIdentity(record), relationship);
	}
	...
	findRelatedRecords(record: RI, relationship: string): FindRelatedRecordsTerm {
		return new FindRelatedRecordsTerm(this.normalizeRecordIdentity(record), relationship);
	}
